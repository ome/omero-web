...
$(function () {
  if (typeof window.OME === "undefined") {
    window.OME = {};
  }
  ...
  window.OME.handleChgrp = function (webindex, staticurl) {
    ...
    // gid, gname, oid
    $chgrpform.dialog({ title: "Move to Group", height: 450, width: 400 });
    ...
    $move_group_tree = $("<div id='move_group_tree'></div>").appendTo(
      $chgrpform
    );
    ...
    $.getJSON(url, function (data) {
      ...
      var ownernames = [];
      for (var o = 0; o < data.owners.length; o++) {
        ownernames.push(data.owners[o][1]);
      }
      var headerTxt =
        "<p>Move data owned by " +
        ownernames.join(", ") +
        ".</p>" +
  ...
  var checkFilesetSplit = function checkFilesetSplit() {
    ...
    $.get(webindex_url + "fileset_check/chgrp/?" + sel, function (html) {
      ...
      if ($("div.split_fileset", html).length > 0) {
        ...
        $(
          ".chgrp_confirm_dialog .ui-dialog-buttonset button:nth-child(2) span"
        ).text("Move All");
        var filesetIds = [];
  ...
  var dryRun = function (targetObjects, groupId) {
    ...
    var msg =
      "<p style='margin-bottom:0'><img alt='Loading' src='" +
      static_url +
      "../webgateway/img/spinner.gif'> " +
        ...
        $.get(url, data, function (dryRunData) {
          if (dryRunData.finished) {
            ...
            if (dryRunData.error) {
              ...
              var errHtml =
                "<img style='vertical-align: middle; position:relative; top:-3px' src='" +
                static_url +
                "../webgateway/img/failed.png'> ";
            ...
            ["Datasets", "Plates", "Images", "Tags", "Files"].forEach(function (
              otype
            ) {
            ...
            if (unlink.length > 0) {
              html +=
                "<br><b style='font-weight: bold'>Not included:</b> " +
                unlink.join(", ");
            }
  ...
  var newContainer = function newContainer() {
    ...
    $(
      "<p>New " +
        ...
        " name: <input name='new_container_name'/></p>"
    )
      .appendTo($chgrpform)
      .click();
    // Hidden input
  ...
  $chgrpform.dialog({
    ...
    buttons: {
      ...
      OK: function () {
        ...
        if (
          $(".split_filesets_info .split_fileset", $chgrpform).length > 0 &&
          $thisBtn.text() == "Move All"
        ) {
          $("#group_chooser").show();
  ...
  $chgrpform.ajaxForm({
    beforeSubmit: function (data, $form) {
      ...
      if (chgrp_target.length == 1) {
        data.push({
          name: "target_id",
          value: chgrp_target.parent().attr("id"),
        });
      }
    },
    success: function (data) {
        ...
        $.each(ids, function (index, id) {
          ...
          if (removeLocated) {
            $.each(removeLocated, function (index, val) {
              if (
                unremovedParentClosure !== undefined &&
      ...
      $.each(removalClosure, function (index, node) {
        ...
        var data = { node: node, old_parent: inst.get_parent(node) };
