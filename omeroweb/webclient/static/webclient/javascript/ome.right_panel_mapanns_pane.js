...
var MapAnnsPane = function MapAnnsPane($element, opts) {
  ...
  this.apiAnnotationUrl = function apiAnnotationUrl(url) {
    if (!url) {
      return WEBCLIENT.URLS.webindex + "api/annotations/";
    }
    return url;
  };
    ...
    if ($mapAnnContainer.is(":visible")) {
      ...
      $.ajax({
        ...
        success: function (data) {
          ...
          if (batchAnn) {
            my_client_map_annotations = groupDuplicateAnns(
              my_client_map_annotations
            );
          ...
          html =
            html +
            mapAnnsTempl({
              anns: my_client_map_annotations,
              objCount: objects.length,
              showTableHead: showHead,
              showNs: false,
              clientMapAnn: true,
