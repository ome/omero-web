...
var FileAnnsPane = function FileAnnsPane($element, opts) {
  ...
  if ($("#choose_attachments_form").length === 0) {
    $(
      "<form id='choose_attachments_form' title='Choose attachments' " +
        ...
        "annotate_file/' method='post'></form>"
    )
      .hide()
      .appendTo("body");
  }
  ...
  $("#choose_file_anns").click(function () {
    ...
    $attach_form.html(
      "&nbsp<br><img src='" +
        WEBCLIENT.URLS.static_webgateway +
        "img/spinner.gif' /> Loading attachments"
    );
  ...
  this.render = function render() {
    if ($fileanns_container.is(":visible")) {
      ...
      $.getJSON(
        WEBCLIENT.URLS.webindex + "api/annotations/?type=file&" + request,
        function (data) {
          ...
          var anns = data.annotations.map(function (ann) {
            ...
            ann.file.size =
              ann.file.size !== null ? ann.file.size.filesizeformat() : "";
            return ann;
          });
          ...
          if (objects.length > 1) {
            ...
            anns.forEach(function (ann) {
              ...
              summary[annId].canRemove =
                summary[annId].canRemove || l.permissions.canDelete;
          ...
          if (checkboxesAreVisible) {
            $(
              "#fileanns_container input[type=checkbox]:not(:visible)"
            ).toggle();
          }
