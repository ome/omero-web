...
var RatingsPane = function RatingsPane($element, opts) {
  ...
  $("#rating_annotations.canAnnotate").on("click", ".myRating img", function (
    event
  ) {
    var $rating = $(this),
  ...
  $("#rating_annotations.canAnnotate").on("click", ".removeRating", function (
    event
  ) {
  ...
  var setRating = function (rating, $rating) {
    ...
    if ($rating) {
      var rating_src =
        WEBCLIENT.URLS.static_webclient + "image/rating" + rating + ".png";
  ...
  this.render = function render() {
    if ($rating_annotations.is(":visible")) {
      ...
      $.getJSON(
        WEBCLIENT.URLS.webindex + "api/annotations/?type=rating&" + request,
        function (data) {
