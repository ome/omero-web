...
var TagPane = function TagPane($element, opts) {
  ...
  if ($("#add_tags_form").length === 0) {
    $(
      "<form id='add_tags_form' title='Tags Selection' action='" +
        WEBCLIENT.URLS.webindex +
        "annotate_tags/' method='post'>"
    )
      .hide()
      .appendTo("body");
  }
  ...
  var showSpinner = function () {
    ...
    $tags_container.append(
      '<img src="' + WEBCLIENT.URLS.static_webgateway + 'img/spinner.gif">'
    );
  };
  ...
  this.render = function render() {
    if ($tags_container.is(":visible")) {
      ...
      var annsUrl =
        WEBCLIENT.URLS.webindex + "api/annotations/?type=tag&" + request;
      ...
      $.getJSON(annsUrl, function (data) {
        ...
        if (objects.length > 1) {
          ...
          tags.forEach(function (tag) {
            ...
            summary[tagId].canRemove =
              summary[tagId].canRemove || l.permissions.canDelete;
