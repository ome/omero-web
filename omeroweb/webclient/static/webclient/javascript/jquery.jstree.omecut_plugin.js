...
(function ($, undefined) {
  ...
  $.jstree.plugins.omecut = function (options, parent) {
    ...
    this.cut = function (obj) {
      ...
      if (
        firstParent.type !== "orphaned" &&
        firstParent.type !== "experimenter"
      ) {
          ...
          $.when(unlinkNode(inst, node, parent)).done(function (rsp) {
            ...
            if (parent.type in rsp) {
              for (var pid in rsp[parent.type]) {
                ...
                if (
                  node.type in children &&
                  children[node.type].indexOf(node.data.obj.id) > -1
                ) {
                  orphaned = false;
                }
              }
            ...
            if (orphaned && !wrongTree) {
              ...
              if (
                ...
                newParent = inst.locate_node(
                  "orphaned-" + ownerExperimenter.data.obj.id
                )[0];
              }
            ...
            var data = { node: node, old_parent: parent };
    ...
    this.paste = function (obj, pos) {
      ...
      if (omecut && ccp_node.length > 0) {
        ...
        $.each(ccp_node, function (index, new_node_data) {
          ...
          var located = inst.locate_node(
            new_node_data.type + "-" + new_node_data.data.obj.id
          );
          if (located.length === 1) {
    ...
    this.check = function (chk, obj, par, pos, more) {
      ...
      if (omecut && obj instanceof Object && !("inst" in obj)) {
        var inst = this;
