...
var linkObjects = function (input) {
  var regex = /(Project|Dataset|Image|Screen|Plate|Well|ROI)(?: ID)*:(?: )*([0-9]+)/gi;
...
OME.well_selection_changed = function ($selected, well_index, plate_class) {
  ...
  $selected.each(function (i) {
    selected_objs.push({
      ...
      index: well_index,
      class: plate_class,
    }); // assume every well has same permissions as plate
  });
...
OME.removeItem = function (event, domClass, url, parentId) {
  ...
  var confirm_remove = OME.confirm_dialog(r + dType + "?", function () {
...
OME.deleteItem = function (event, domClass, url) {
  ...
  var $parent = $(event.target).parents("." + domClass);
  var $annContainer = $parent.parent();
  var confirm_remove = OME.confirm_dialog("Delete " + dType + "?", function () {
    if (confirm_remove.data("clicked_button") == "OK") {
      $.ajax({
        type: "POST",
        url: url,
...
OME.refreshThumbnails = function (options) {
  ...
  if (
    ...
    OME.preview_viewport &&
    OME.preview_viewport.loadedImg.id
  ) {
        ...
        success: function (data) {
          ...
          $.each(data, function (key, value) {
            if (value !== null) {
              ...
              $("#wellImages li[data-imageId='" + key + "'] img").attr(
                "src",
                value
              );
              $("#well_birds_eye img[data-imageid='" + key + "']").attr(
                "src",
                value
              );
              // Search results
          ...
          if (
            invalid_thumbs.length < iids.length ||
            (batch === 1 && invalid_thumbs.length === 1)
          ) {
...
OME.handleDelete = function (deleteUrl, filesetCheckUrl, userId) {
  ...
  del_form.dialog("open").removeData("clicked_button");
  ...
  for (var key in dtypes) {
    ...
    type_strings.push(
      key.replace("acquisition", "Run").capitalize() +
        ((dtypes[key] > 1 && "s") || "")
    );
  }
      ...
      $.ajax({
        ...
        success: function (r) {
          ...
          $.each(selected, function (index, node) {
            ...
            var data = { node: node, old_parent: firstParent };
            update_thumbnails_panel(e, data);
          });
  ...
  var $deleteYesBtn = $(
      ".delete_confirm_dialog .ui-dialog-buttonset button:nth-child(1)"
    ),
    $deleteNoBtn = $(
      ".delete_confirm_dialog .ui-dialog-buttonset button:nth-child(2) span"
    );
...
OME.nodeHasPermission = function (node, permission) {
  ...
  if (permission === "isOwned") {
    if (
      node.data.obj.hasOwnProperty("ownerId") &&
      node.data.obj.ownerId === currentUserId()
    ) {
      return node.data.obj.isOwned;
    } else if (
      node.type === "experimenter" &&
      node.data.id == currentUserId()
    ) {
      return true;
    }
    return false;
  ...
  if ($("#scriptList li").length === 0) {
    ...
    $.get(
      ...
      function (data) {
        var build_ul = function (script_data) {
          ...
          for (var i = 0; i < script_data.length; i++) {
            ...
            if (li.id) {
              ...
              html +=
                "<ul><li class='menu_back'><a href='#'>back</a></li>" +
                build_ul(li.ul) +
                "</ul>";
              html += "</li>";
            }
        ...
        if (
          WEBCLIENT.current_admin_privileges.indexOf("WriteScriptRepo") > -1
        ) {
          html +=
            "<li><a class='upload_script' href='" +
            WEBCLIENT.URLS.script_upload +
            "'>Upload Script</a></li>";
        }
...
OME.fileAnnotationCheckboxDynamicallyAdded = function () {
  var checkboxesAreVisible =
    $("#fileanns_container input[type=checkbox]:visible").length > 0;
...
OME.applyRenderingSettings = function (rdef_url, selected) {
  ...
  var confirmMsg =
    "This will save new rendering settings to " +
    selected.length +
    " " +
    type +
    (selected.length > 1 ? "s" : "") +
