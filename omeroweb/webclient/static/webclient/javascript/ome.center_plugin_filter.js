function MapAnnFilter(image_ids, $element, callback, filterObjects) {
  ...
  var $filter = $(
    '<div class="imagefilter filtersearch" style="width:100%">' +
  ...
  $(".choose_map_key", $filter).change(
    function (event) {
      ...
      if (this.currentFilterKey == "-") {
        ...
        this.currentKeyValues = this.usedKeyValues[
          this.currentFilterKey
        ].values;
        this.keyisNumber =
          this.usedKeyValues[this.currentFilterKey].type === "number";
...
MapAnnFilter.prototype.isImageVisible = function (iid) {
  ...
  if (!this.currentKeyValues) {
    ...
    this.currentKeyValues[iid].forEach(
      function (v) {
        v = parseFloat(v);
