...
$(function () {
  ...
  var jstree = $("#dataTree")
    ...
    .on("copy_node.jstree", function (e, data) {
      ...
      // Persist
      if (!childExists) {
        $.when(linkNode(inst, data.node, inst.get_node(data.parent))).done(
          function () {
      ...
      updateParentInsertNode(
        inst,
        data.node,
        inst.get_node(data.parent),
        data.position
      );
    })
    .on("move_node.jstree", function (e, data) {
      ...
      if (
        data.parent.type !== "experimenter" &&
        data.parent.type !== "orphaned" &&
        !childExists
      ) {
        linkPromise = linkNode(inst, data.node, inst.get_node(data.parent));
      }
      var unlinkPromise = unlinkNode(
        inst,
        data.node,
        inst.get_node(data.old_parent)
      );
    ...
    .on("loaded.jstree", function (e, data) {
      ...
      if (nodeIds.length === 0) {
        ...
        nodeIds = nodeIds.filter(function (n) {
          return !n.startsWith("well");
        });
    ...
    .on("dblclick.jstree", ".jstree-anchor", function (e) {
      ...
      if (node) {
        if (node.type === "image") {
          ...
          var url =
            WEBCLIENT.URLS.webindex + "img_detail/" + node.data.obj.id + "/";
    ...
    .on("refresh.jstree", function () {
      ...
      $.each(refreshPathsReverse, function (index, refreshPathReverse) {
        ...
        var locatedNodes = datatree.locate_node(
          refreshPathReverse[0][0] + "-" + refreshPathReverse[0][1]
        );
        $.each(locatedNodes, function (index, node) {
          ...
          $.each(refreshPathReverse, function (index, pathComponent) {
            if (
              ...
              traverseNode = datatree.get_node(
                datatree.get_parent(traverseNode)
              );
            } else {
              matched = false;
    ...
    .jstree({
      ...
      conditionalselect: {
        ...
        conditionalselect_function: function (node) {
          ...
          $.each(selected, function (index, sel) {
            if (
              sel.type === node.type &&
              sel.data.obj.id === node.data.obj.id
            ) {
              allowSelect = false;
              // Break out of each
              return false;
      ...
      core: {
        ...
        data: function (node, callback, payload) {
          ...
          if (node.hasOwnProperty("data") && node.type != "orphaned") {
            ...
            var tagroot =
              WEBCLIENT.URLS.tree_top_level ===
                WEBCLIENT.URLS.api_tags_and_tagged &&
          ...
          if (
            path &&
            (node.type === "experimenter" || node.type === "orphaned")
          ) {
        ...
        check_callback: function (
          ...
          if (operation === "copy_node" || operation === "move_node") {
            ...
            if (
              oldParent.type === node_parent.type &&
          ...
          if (
            ...
            if (
              !OME.nodeHasPermission(node_parent, "canLink") &&
      ...
      types: {
        ...
        map: {
          icon:
            WEBCLIENT.URLS.static_webclient + "image/left_sidebar_icon_map.png",
          valid_children: ["project", "screen"],
          draggable: false,
        },
        tagset: {
          icon:
            WEBCLIENT.URLS.static_webclient +
            "image/left_sidebar_icon_tags.png",
          valid_children: ["tagset", "tag"],
        },
        tag: {
          icon:
            WEBCLIENT.URLS.static_webclient + "image/left_sidebar_icon_tag.png",
          valid_children: [
            "project",
            "dataset",
            "image",
      ...
      contextmenu: {
        ...
        items: function (node) {
          ...
          if (tagTree) {
            config["create"]["submenu"] = {
              ...
              tag: {
                ...
                icon:
                  WEBCLIENT.URLS.static_webclient +
                  "image/left_sidebar_icon_tag.png",
                action: function (node) {
            ...
            config["create"]["submenu"] = {
              ...
              dataset: {
                ...
                _disabled: true,
                icon:
                  WEBCLIENT.URLS.static_webclient + "image/folder_image16.png",
                action: function (node) {
              ...
              screen: {
                ...
                _disabled: true,
                icon:
                  WEBCLIENT.URLS.static_webclient + "image/folder_screen16.png",
                action: function (node) {
          ...
          config["delete"] = {
            ...
            icon:
              WEBCLIENT.URLS.static_webclient +
              "image/icon_basic_delete_16.png",
            action: function () {
          ...
          config["chgrp"] = {
            ...
            _disabled: true,
            icon:
              WEBCLIENT.URLS.static_webclient + "image/icon_basic_user_16.png",
            action: function () {
              // TODO - make sure this works with new jsTree
              OME.handleChgrp(
                WEBCLIENT.URLS.webindex,
                WEBCLIENT.URLS.static_webclient
              );
            },
          };
          ...
          config["share"] = {
            ...
            icon:
              WEBCLIENT.URLS.static_webclient + "image/icon_toolbar_share2.png",
            action: function () {
          ...
          config["renderingsettings"] = {
            ...
            submenu: {
              copy_rdef: {
                ...
                action: function () {
                  var inst = $.jstree.reference("#dataTree");
                  OME.copyRenderingSettings(
                    WEBCLIENT.URLS.copy_image_rdef_json,
              ...
              reset_rdef: {
                ...
                action: function () {
                  var inst = $.jstree.reference("#dataTree");
                  OME.resetRenderingSettings(
                    WEBCLIENT.URLS.reset_rdef_json,
                    inst.get_selected(true)
                  );
                },
              },
              owner_rdef: {
                ...
                action: function () {
                  var inst = $.jstree.reference("#dataTree");
                  OME.applyOwnerRenderingSettings(
                    WEBCLIENT.URLS.reset_owners_rdef_json,
                    inst.get_selected(true)
                  );
                },
              },
            ...
            var viewers = WEBCLIENT.OPEN_WITH.map(function (v) {
              return {
                ...
                _disabled: function () {
                  ...
                  if (
                    typeof v.supported_objects === "object" &&
                    v.supported_objects.length > 0
                  ) {
                    enabled = v.supported_objects.reduce(function (
                      prev,
                      supported
                    ) {
          ...
          var userId = WEBCLIENT.active_user.id,
            ...
            memberOfGroup =
              WEBCLIENT.eventContext.memberOfGroups.indexOf(
                WEBCLIENT.active_group_id
              ) > -1,
            writeOwned =
              WEBCLIENT.eventContext.adminPrivileges.indexOf("WriteOwned") > -1,
            allMembers = userId === -1,
            // canCreate if looking at your own data or 'All Members' OR User's data && writeOwned
            canCreate =
              userId === WEBCLIENT.USER.id ||
              (allMembers && memberOfGroup) ||
              (!allMembers && writeOwned),
            canLink = OME.nodeHasPermission(node, "canLink"),
            parentAllowsCreate =
              node.type === "orphaned" || node.type === "experimenter";
          ...
          if (canCreate) {
            ...
            if (tagTree) {
              ...
              if (node.type === "project") {
                ...
                config["create"]["submenu"]["dataset"]["_disabled"] = !(
                  canLink || writeOwned
                );
              } else {
          ...
          if (canLink) {
            ...
            var canCut =
              ["dataset", "image", "plate", "tag"].indexOf(node_type) > -1;
            ...
            var canCopy =
              (node_type === "dataset" && parent_type === "project") ||
            ...
            var canPaste =
              (node_type === "project" && to_paste === "dataset") ||
          ...
          if (OME.nodeHasPermission(node, "canAnnotate")) {
            if (
              ...
              if (WEBCLIENT.HAS_RDEF) {
                ...
                config["renderingsettings"]["submenu"]["paste_rdef"][
                  "_disabled"
                ] = false;
              }

              config["renderingsettings"]["_disabled"] = false;
              config["renderingsettings"]["submenu"]["reset_rdef"][
                "_disabled"
              ] = false;
              config["renderingsettings"]["submenu"]["owner_rdef"][
                "_disabled"
              ] = false;
            }
          }
          ...
          if (node.type === "image") {
            config["renderingsettings"]["_disabled"] = false;
            config["renderingsettings"]["submenu"]["copy_rdef"][
              "_disabled"
            ] = multiselection;
          }
          return config;
