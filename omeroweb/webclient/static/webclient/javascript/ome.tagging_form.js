...
var tagging_form = function (
  ...
  var progressbar = $("#add_tags_progress .progress-striped").progressbar({
    change: function () {
      $("#add_tags_progress .progress-value").text(
        $("#add_tags_progress .progress-striped").progressbar("value") + "%"
      );
    },
    complete: function () {
      setTimeout(function () {
  ...
  var get_selected_tagset = function () {
    ...
    if (
      ...
      return selected.hasClass("alltags-tagset")
        ? selected
        : selected.prevAll(".alltags-tagset").eq(0);
    } else {
      return null;
    }
  };
  ...
  var update_selected_labels = function () {
    var count = 0;
    $(".ui-selected", div_all_tags)
      .not(".filtered")
      .each(function () {
        var $this = $(this);
        if ($this.hasClass("alltags-tagset")) {
          count += $this
            .nextUntil(":not(.alltags-childtag)")
            .not(".filtered, .ui-selected").length;
        } else {
          count++;
        }
      });
    ...
    if (tagset) {
      $("#id_selected_tag_set").html(
        "Add a new tag in <span class='tagset-title'>" +
          tagset.text() +
          "</span> tag set and select it immediately:"
      );
    } else {
  ...
  var loader = function () {
    // wrapper function for all data loading code
    ...
    var load = function (mode, callback, offset, limit) {
      ...
      url = url + "?jsonmode=" + mode + "&group=" + WEBCLIENT.active_group_id;
    ...
    var tags_callback = function (data) {
      ...
      progressbar.progressbar(
        "value",
        Math.ceil(num_tag_callbacks * step_weight)
      );
    };
    ...
    var owners_callback = function (data) {
      ...
      progressbar.progressbar(
        "value",
        Math.ceil((batch_steps + 1) * step_weight)
      );
    ...
    var desc_callback = function (data) {
      ...
      if (++num_desc_callbacks === batch_steps) {
        ...
        progressbar.progressbar(
          "value",
          Math.ceil((batch_steps + 1 + num_desc_callbacks) * step_weight)
        );
      }
    };
    ...
    var process_tags = function () {
      ...
      for (idx in selected_tags) {
        ...
        var selected_tag = $(
          ".tag_selection div[data-id=" + selected_tags[idx][0] + "]"
        );
      ...
      for (idx in others_tags) {
        var tag_to_update = $(
          ".tag_selection div[data-id=" + others_tags[idx] + "]"
        );
        if (tag_to_update.length) {
          var others_count =
            parseInt(tag_to_update.attr("data-others_count") || "0", 10) + 1;
          tag_to_update.attr("data-others_count", others_count);
          tag_to_update.attr(
            "data-linkownername",
            others_count
              ? others_count + " other user" + (others_count > 1 ? "s" : "")
              : selected_tags[idx][2]
          );
        }
      }
  ...
  var select_tags = function (event) {
    ...
    $(
      "div.ui-selected.alltags-tag:not(.filtered), " +
        "div.ui-selected.alltags-childtag:not(.filtered)",
      div_all_tags
    )
      .each(function () {
    ...
    if (first_selected.length > 0) {
      div_selected_tags
        .parent()
        .scrollTop(
          first_selected.offset().top - div_selected_tags.offset().top - 40
        );
    }
    enable_buttons();
  ...
  var deselect_tags = function (event) {
    ...
    $("div.ui-selected", div_selected_tags)
      .each(function () {
    ...
    if (first_selected.length > 0) {
      div_all_tags
        .parent()
        .scrollTop(
          first_selected.offset().top - div_all_tags.offset().top - 40
        );
    }
    enable_buttons();
    event.preventDefault();
  ...
  var update_filter = function (event, keep_selection) {
    ...
    var cleanup = function () {
      ...
      if (!no_filter) {
        ...
        $("div.alltags-tagset", div_all_tags).each(function () {
          $(this).toggleClass(
            "filtered",
            !unfiltered_tagsets[this.getAttribute("data-id")]
          );
        });
      }
      if (!keep_selection) {
        $("div.ui-selected", div_all_tags).removeClass("ui-selected");
        if (filters !== "") {
          $("div.alltags-tag,div.alltags-childtag", div_all_tags)
            .not(".filtered")
            .first()
            .addClass("ui-selected");
        }
    ...
    var owner_mode = $("select[name=filter_owner_mode] option:selected").val();
        ...
        for (var idx = pos; idx < endpos; idx++) {
          ...
          if (mode === "any") {
            for (var filter in filters_split) {
              match = match && text.indexOf(filters_split[filter]) >= 0;
            }
          } else {
  ...
  var add_new_tag = function (event, force) {
    ...
    if (text.length > 0) {
      ...
      all_tags[new_tag_counter] = {
        ...
        s: tagset_id,
        sort_key:
          (tagset_id ? all_tags[tagset_id].t.toLowerCase() : "") +
          text.toLowerCase(),
      };
      var div = $(
        create_tag_html(
  ...
  var save_tags = function () {
    ...
    $("div", div_selected_tags).each(function () {
      ...
      if (tag_id[0] === "-") {
        ...
        new_tags.after(
          $("<input type='hidden' />")
            .attr("name", "newtags-" + count + "-tag")
            .val($(this).text())
        );
        new_tags.after(
          $("<input type='hidden' />")
            .attr("name", "newtags-" + count + "-description")
            .val(this.getAttribute("data-description"))
        );
        new_tags.after(
          $("<input type='hidden' />")
            .attr("name", "newtags-" + count + "-tagset")
    ...
    $("div", list)
      .sort(function (a, b) {
        return all_tags[a.getAttribute("data-id")].sort_key >
          all_tags[b.getAttribute("data-id")].sort_key
          ? 1
          : -1;
      })
      .appendTo(list);
  ...
  tag_input
    .keyup(update_add_new_button_state)
    .change(update_add_new_button_state);
  update_add_new_button_state();
