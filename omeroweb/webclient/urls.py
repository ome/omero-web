...
urlpatterns = [
    ...
    re_path(r'^(?P<menu>((?i)userdata|public|history|search|help|usertags))/$',
        views.load_template,
        name="load_template"),
    re_path(r'^userdata/$',
    ...
    re_path(r'^activities_json/',
        views.activities,
        {'template': 'json'},
        name="activities_json"),
    re_path(r'^activities_update/(?:(?P<action>clean)/)?$',
        views.activities_update,
        name="activities_update"),

    # loading data
    re_path(r'^load_plate/(?:(?P<o1_type>'
    ...
    # data
    re_path(r'^fileset_check/(?P<action>((?i)delete|chgrp))/$',
        views.fileset_check,
        name="fileset_check"),
    ...
    re_path(r'^download_placeholder/$', views.download_placeholder,
    ...
    # annotations
    re_path(r'^action/(?P<action>[a-zA-Z]+)/(?:(?P<o_type>[a-zA-Z]+)/)'
        r'?(?:(?P<o_id>[0-9]+)/)?$',
    ...
    re_path(r'^marshal_tagging_form_data/$', views.marshal_tagging_form_data,
        name="marshal_tagging_form_data"),
    re_path(r'^annotate_rating/$',
        views.annotate_rating,
        name="annotate_rating"),
    re_path(r'^annotate_comment/$',
    ...
    re_path(r'^annotation/(?P<annId>[0-9]+)/$',
        views.download_annotation,
        name="download_annotation"),
    re_path(r'^load_original_metadata/(?P<imageId>[0-9]+)/'
        r'(?:(?P<share_id>[0-9]+)/)?$',
        views.load_original_metadata,
        name="load_original_metadata"),
    re_path(r'^download_orig_metadata/(?P<imageId>[0-9]+)/$',
        views.download_orig_metadata,
        name="download_orig_metadata"),
    re_path(r'^omero_table/(?P<file_id>[0-9]+)/(?:(?P<mtype>((?i)json|csv))/)?$',
        views.omero_table,
        name="omero_table"),
    ...
    re_path(r'^ome_tiff_script/(?P<imageId>[0-9]+)/$',
        views.ome_tiff_script,
        name='ome_tiff_script'),
    re_path(r'^ome_tiff_info/(?P<imageId>[0-9]+)/$',
        views.ome_tiff_info,
        name='ome_tiff_info'),
    ...
    re_path(r'^api/groups/$', views.api_group_list,
        name='api_groups'),

    re_path(r'^api/experimenters/(?P<experimenter_id>-?\d+)/$',
    ...
    re_path(r'^api/share_images/(?P<share_id>[0-9]+)/$', views.api_image_list,
    ...
    re_path(r'^api/plate_acquisitions/$', views.api_plate_acquisition_list,
    ...
    re_path(r'^api/paths_to_object/$', views.api_paths_to_object,
        name='api_paths_to_object'),

    # Get parents of 1 or more objects. ?image=1,2&dataset=3
    re_path(r'^api/parent_links/$', views.api_parent_links,
        name='api_parent_links'),

    re_path(r'^api/tags/$', views.api_tags_and_tagged_list,
        name='api_tags_and_tagged'),

    re_path(r'^api/annotations/$', views.api_annotations,
