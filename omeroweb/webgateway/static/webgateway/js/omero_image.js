(function () {
  ...
  window.copyRdefs = function (viewport) {
    ...
    // save to session
    var jqxhr = $.getJSON(
      viewport.viewport_server + "/copyImgRDef/?" + rdefQry
    );
  ...
  window.pasteRdefs = function (viewport) {
    var doPaste = function (data) {
      ...
      if (
        channels.length != viewport.getChannels().length ||
      ...
      var pasteData = { c: data.c, m: data.m, maps: data.maps };
    ...
    $.getJSON(viewport.viewport_server + "/getImgRDef/", function (data) {
      if (data.rdef) {
        doPaste(data.rdef);
      }
    });
  ...
  window.resetImageDefaults = function (viewport, obj, callback) {
    ...
    $.getJSON(
      viewport.viewport_server +
        "/imgData/" +
        viewport.loadedImg.id +
        "/?getDefaults=true",
      function (data) {
  ...
  window.setImageDefaults = function (viewport, obj, callback, skip_apply) {
    ...
    gs_modalJson(
      viewport.viewport_server +
        ...
        viewport.loadedImg.id +
        "/?" +
        viewport.getQuery(true),
      {},
      function (success, rv) {
        $(obj).html(old).prop("disabled", false);
        if (!(success && rv)) {
          alert(
            "Setting image defaults failed. Success: " +
              success +
              " Response: " +
              rv
          );
  ...
  window.syncChannelsActive = function (viewport) {
    ...
    for (i = 0; i < channels.length; i++) {
      ...
      if ($chbx.length > 0) {
        // in case this is called before UI is built
  ...
  window.syncRDCW = function (viewport) {
    ...
    for (i = 0; i < channels.length; i++) {
      ...
      $("#wblitz-ch" + i + "-cw-start")
        .val(channels[i].window.start)
        .change();
      $("#wblitz-ch" + i + "-cw-end")
        .val(channels[i].window.end)
        .change();
    }
  ...
  window.applyRDCW = function (viewport, final) {
    ...
    for (var i = 0; i < viewport.getCCount(); i++) {
      ...
      viewport.setChannelColor(
        i,
        $("#wblitz-ch" + i + "-color").attr("data-color"),
        true
      );
      revInt = $("#wblitz-ch" + i + "-color").data("data-reverse-intensity");
      if (revInt !== undefined) {
        viewport.setChannelInverted(i, revInt, true);
      }
      var noreload = i + 1 < viewport.getCCount(); // prevent reload, except on the last loop
      viewport.setChannelWindow(
        i,
        $("#wblitz-ch" + i + "-cw-start").get(0).value,
        $("#wblitz-ch" + i + "-cw-end").get(0).value,
        noreload
      );
    }
  ...
  window._load_metadata = function (ev, viewport) {
    ...
    $("#wblitz-image-description-content").html(
      tmp.imageDescription.replace(/\n/g, "<br />")
    );
  ...
  window.loadBulkAnnotations = function (url, query, callback) {
    ...
    $.getJSON(url + "?query=" + query + "&callback=?", function (result) {
      if (result.data && result.data.rows) {
        var table = $("#bulk-annotations")
          .show()
          .next()
          .show()
          .children("table");
  ...
  window._refresh_cb = function (ev, viewport) {
    ...
    var doToggle = function (index) {
      return function () {
        ...
        viewport.save_channels();
        updateUndoRedo(viewport);
        viewport.self.trigger("channelToggle", [
          viewport,
          index,
          viewport.loadedImg.channels[index],
        ]);
      };
    };
    for (i = 0; i < channels.length; i++) {
      $(
        '<button id="wblitz-ch' +
          i +
          '" class="squared' +
          (channels[i].active ? " pressed" : "") +
          '" style="background-color: #' +
          channels[i].color +
          '" title="' +
          channels[i].label +
          '"><div class="lutBackground"></div><div class="btnLabel">' +
          channels[i].label +
          "</div></button>"
      )
        .appendTo(box)
    ...
    $("#wblitz-image-pixel-size-x").html(
      tmp.x === null ? "-" : tmp.x.lengthformat()
    );
    $("#wblitz-image-pixel-size-y").html(
      tmp.y === null ? "-" : tmp.y.lengthformat()
    );
    $("#wblitz-image-pixel-size-z").html(
      tmp.z === null ? "-" : tmp.z.lengthformat()
    );
    ...
    $("#rdef-postit table").on("focus", ".rangewidget input", function () {
      ...
      viewport.self.trigger("channelFocus", [
        viewport,
        chIdx,
        viewport.loadedImg.channels[i],
      ]);
    });
    ...
    var slide_start_cb = function () {
      ...
      return function (event, ui) {
        $(this)
          .data("channel_start", ui.values[0])
    ...
    var slide_cb = function () {
      return function (event, ui) {
        ...
        if (ui.values[0] !== s) {
          $("#wblitz-ch" + $(event.target).data("channel-idx") + "-cw-start")
            .val(ui.values[0])
            .change();
        } else if (ui.values[1] !== e) {
          $("#wblitz-ch" + $(event.target).data("channel-idx") + "-cw-end")
            .val(ui.values[1])
            .change();
        }
        viewport.self.trigger("channelSlide", [
          viewport,
          $(event.target).data("channel-idx"),
          ui.values[0],
          ui.values[1],
        ]);
      };
    ...
    for (i = channels.length - 1; i >= 0; i--) {
      ...
      $("#wblitz-ch" + i + "-cw").append(
        '<td width="10%"><span class="min" title="min: ' +
          channels[i].window.min +
          '"><input type="text" id="wblitz-ch' +
          i +
          '-cw-start" /></span></td><td><div class="rangeslider" id="wblitz-ch' +
          i +
          '-cwslider"></div></td> <td width="10%"><span class="max" title="max: ' +
