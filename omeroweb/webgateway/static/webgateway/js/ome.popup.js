...
OME.isMobileDevice = function () {
  return /Android|webOS|iPhone|iPad|iPod|BlackBerry|PlayBook|IEMobile|Opera Mini|Mobile Safari|Silk/i.test(
    navigator.userAgent
  );
};
...
jQuery.fn.numbersOnly = function (options) {
  ...
  this.each(function () {
    ...
    if (
      ...
      allowedChars.indexOf(charCode) == -1
    ) {
      return false;
    }
...
OME.openCenteredWindow = function (url, w, h) {
  ...
  var windowFeatures =
    ...
    ",height=" +
    height +
    ",status=no,resizable=yes,scrollbars=yes,menubar=no,toolbar=no,left=" +
    left +
    ",top=" +
    top +
    "screenX=" +
...
OME.login_dialog = function (login_url, callback) {
  ...
  $dialog.html(
    "<form>Username:<input type='text' name='username' id='login_username' /><br />Password:<input type='text' name='password' id='login_password'/>"
  );
  ...
  $dialog.dialog({
    ...
    buttons: {
      ...
      Connect: function () {
        ...
        $.post(
          login_url,
          { password: password, username: username, noredirect: "true" },
          function (data) {
...
(function ($) {
  ...
  $.fn.omeroweb_right_plugin = function (settings) {
    ...
    this.each(function (i) {
      ...
      var load_plugin_content =
          settings["load_plugin_content"] || settings["load_tab_content"],
      ...
      $("body").bind("selection_change.ome", function (event) {
        ...
        if (typeof supported_obj_types != "undefined") {
          supported =
            $.inArray(orel, supported_obj_types) > -1 && selected.length == 1;
        } else {
  ...
  $.fn.omeroweb_center_plugin = function (settings) {
    ...
    this.each(function (i) {
      ...
      $("#center_panel_chooser").bind(
        "center_plugin_changed.ome",
...
if (false) {
  // set to 'true' to run. NB: Need to uncomment 'console.log..' below.
  setTimeout(function () {
    // use timeout to allow all scripts to load etc
    ...
    for (i in differences) {
      ...
      } else if (globals[differences[i].type]) {
        delete differences[i];
      }
    }
    ...
    exceptions.push(
      ...
      "rgbToHex",
      "parseQuery",
      "downloadLandingDialog"
    ); // from ome.gs_utils.js
