...
function gs_loadBlockUI(callback) {
  if (jQuery.blockUI === undefined) {
    jQuery.getScript(
      gs_static_location_prefix + "3rdparty/jquery.blockUI-2.66.0.js",
      callback
    );
    return false;
  }
...
function gs_choiceModalDialog(
  ...
  if (
    !gs_loadBlockUI(function () {
      gs_choiceModalDialog(
        message,
        choices,
        callback,
        blockui_opts,
  ...
  for (var i = 0; i < choices.length; i++) {
    message +=
      '<input type="button" onclick="return gs_modal_cb(' +
      i +
      ');" value="' +
      choices[i].label +
      '" />';
...
function gs_showResultLine(container, data, baseurl, renderurl) {
  ...
  head.append(
    '<a href="' + data.links.paper + '">- ' + data.project + " -</a>"
  );
  head.append(
    '<div class="detail">' +
  ...
  var foot = jQuery(
    '<div class="search-result-footnotes"><span> [ </span></div>'
  ).appendTo(result);
  var fv = jQuery(
    '<a href="' + data.links.viewer + '" alt="Open Full Viewer">Full Viewer</a>'
  ).appendTo(foot);
  foot.append(
    '&nbsp;<a href="' + data.links.paper + '" alt="Paper">Paper</a>&nbsp;'
  );
  fv.click(data.links.fv_click(data.datasetId, data.imageId));
  foot.append(
    '<a href="' + data.links.figure + '" alt="Figure">Figure</a>&nbsp;'
  );
  foot.append(
    "<span>] by <i>" +
      data.author +
...
function gs_searchImgs(text, baseurl, renderurl, result_cb) {
  if (text.length > 0) {
    jQuery("#search-results-summary")
      .removeClass("ajax-error")
      .html('searching for "' + text + '"');
    ...
    $.getJSON(
      baseurl + "search/",
      { text: text, ctx: "imgs", grabData: true, key: "meta" },
      function (data) {
        shown = 0;
        if (data.length) {
          jQuery("#search-results").html("");
          for (e in data) {
            var elm = gs_showResultLine(
        ...
        if (shown == 0) {
          ...
          jQuery("#search-results-summary").html(
            'search for "' + text + '": no results.'
          );
        } else {
          jQuery("#search-results-summary").html(
            'search for "' +
...
function downloadLandingDialog(anchor, msg, cb) {
  ...
  gs_choiceModalDialog(msg, [{ label: "close", data: 1 }], ccb, {
  ...
  if (anchor) {
    ...
    if (!dliframe.length) {
      dliframe = $(
        '<iframe name="dliframe" width="0" height="0"></iframe>'
      ).appendTo("body");
    }
