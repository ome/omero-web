...
jQuery.WeblitzPlateview = function (elm, options) {
  ...
  var rv =
    container.WeblitzPlateview ||
    (container.WeblitzPlateview = new jQuery._WeblitzPlateview(
      container,
      options
    ));
  return rv;
...
jQuery._WeblitzPlateview = function (container, options) {
    ...
    for (i = 0; i < data.rowlabels.length; i++) {
      ...
      for (var j = 0; j < data.grid[i].length; j++) {
        if (data.grid[i][j] === null) {
          html +=
            '<td class="placeholder"><img src="' + spacer_gif_src + '" /></td>';
        } else {
          ...
          html +=
            ...
            parentPrefix +
            "well-" +
            data.grid[i][j].wellId +
            '">' +
            '<img class="waiting" src="' +
            spacer_gif_src +
            '" />' +
            '<div class="wellLabel">' +
            data.rowlabels[i] +
            ...
            data.grid[i][j].id +
            '" class="loading" name="' +
            (data.rowlabels[i] + data.collabels[j]) +
            '"></td>';
        }
      }
      html += "</tr>";
  ...
  this.load = function (pid, field) {
    ...
    $("table div.placeholder", _this.self)
