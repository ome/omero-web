...
jQuery.aop.before(
  { target: jQuery, method: "show" },
  aop_handler("postit_open_handler")
);
jQuery.aop.after(
  { target: jQuery.fn, method: "hide" },
  aop_handler("postit_close_handler")
);
...
$.fn.postit = function (cfg) {
  ...
  return this.each(function () {
    ...
    dragbar.dblclick(function (e) {
      self.toggleClass("collapsed");
    });

    if (dragbar.get(0).addEventListener) {
      // Respond to mouse wheel in Firefox
      dragbar.get(0).addEventListener(
        "DOMMouseScroll",
        function (e) {
          if (e.detail > 0) self.removeClass("collapsed");
          else if (e.detail < 0) self.addClass("collapsed");
          e.preventDefault();
        },
        false
      );
    ...
    self
      ...
      .removeClass("sdialog");
    /* The buttons */
    self.find(".postit-toggle-btn").click(function (e) {
      self.toggleClass("collapsed");
    });
    self.find(".postit-close-btn").click(function (e) {
      self.hide();
    });
    self
      .find(".sdialog-content")
      .removeClass("sdialog-content")
      .addClass("postit-content");
    if (cfg && !cfg.noResize) {
      var target;
