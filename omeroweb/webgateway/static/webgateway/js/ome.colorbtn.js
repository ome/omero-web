...
$.fn.colorbtn = function (cfg) {
  return this.each(function () {
    ...
    var callback = function (color) {
      ...
      jQuery("input#" + self[0].cfg.prefix + "-tb").val(
    ...
    this._prepare_picker = function () {
      ...
      var box = jQuery("#" + this.cfg.prefix + "-box").append(
        "<h1>Choose color</h1>"
      );
      box.postit();

      var invertHtml =
        '<div><input id="invert" type="checkbox" style="width:20px"></input>';
      invertHtml +=
        '<label for="invert" style="font-size:15px; font-weight: normal;">Invert</label></div>';
      invertHtml += '<div style="clear:both"></div>';
      $(invertHtml).appendTo(box);

      // Add Lookup Table list - gets populated in show_picker() below.
      var $luts = $(
        '<div id="' + this.cfg.prefix + '-luts" class="lutpicker"></div>'
      ).appendTo(box);

      // Colorpicker - uses farbtastic.js
      var $showColorPicker = $(
        "<h1 style='font-size:1.5em' class='showColorPicker'><a href='#'>Show Color Picker</a></h1>"
      ).appendTo(box);
      var $cpickerPane = $("<div class='cpickerPane'></div>").appendTo(box);
      $('<div id="' + this.cfg.prefix + '"></div>').appendTo($cpickerPane);
      $cpickerPane.append(
        '<div style="text-align: center;">Hex RGB <input type="text" id="' +
          this.cfg.prefix +
          '-tb" /></div>'
      );
      $(
        '<button id="cbpicker-OK-btn" style="float:right">OK</button>'
      ).appendTo($cpickerPane);
      ...
      jQuery("input#" + this.cfg.prefix + "-tb").bind("change", function () {
        var new_color = sanitizeHexColor(
          jQuery("input#" + self[0].cfg.prefix + "-tb").val()
        );
        if (new_color !== null) {
    ...
    this.show_picker = function () {
      ...
      if ($luts.is(":empty")) {
        ...
        var lutRows = OME.LUTS.map(function (lut) {
          ...
          if (idx > -1) {
            preview =
              'class="lutBackground" style="background-position: 0 -' +
              (idx * 30 + 7) +
              "px;";
      ...
      $("#cbpicker-OK-btn")
        .unbind("click")
        .bind("click", ok_callback)
        .bind("click", function () {
          jQuery("#" + that.cfg.prefix + "-box").hide();
        });
      $("#" + this.cfg.prefix + "-luts")
        .off("click")
      ...
      jQuery("#" + this.cfg.prefix + "-tb").val(
        color.substring(1).toUpperCase()
      );
      jQuery("#" + this.cfg.prefix + "-defc").css(
        "background-color",
        self.css("background-color")
      );
      jQuery("#" + this.cfg.prefix + "-box")
        .mousedown(function () {
