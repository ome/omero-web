...
$.fn.gs_slider = function (cfg) {
  return this.each(function () {
    ...
    slider_container.append(
      '<div id="' + handleId + '" class="slider-handle draggable"></div>'
    );
    ...
    /* The buttons */
    slider.before(
      '<div id="' + btnPlayUpId + '" class="slider-btn-playup"></div>'
    ); //&nbsp;</div>');
    ...
    slider.after(
      '<div id="' + btnPlayDownId + '" class="slider-btn-playdown"></div>'
    ); //&nbsp;</div>');
    ...
    this.setSliderPos = function (pos, trigger) {
      var pos =
        Math.min(Math.max(pos, this.sliderCfg.min), this.sliderCfg.max) -
        this.sliderCfg.min;
      if (pos != this.pos || trigger) {
        ...
        if (self.sliderCfg.range == 1) {
          ...
          handle.css(
            this.sliderCfg.anchor,
            100.0 - handle_rel - (this.pos * 100.0) / this.sliderCfg.range + "%"
          );
        } else {
          handle.css(
            this.sliderCfg.anchor,
            (this.pos * 100.0) / this.sliderCfg.range + "%"
          );
        }
        if (trigger != false) {
    ...
    var startrepeat = function (additive) {
      ...
      var repeat_func = function (timeout) {
        repeat_timer = setTimeout(function () {
          if (onrepeat) {
            if (
              self.sliderCfg.repeatCallback != null &&
              !self.sliderCfg.repeatCallback()
            ) {
    ...
    var domove = function (e) {
      ...
      if (ondrag) {
        ...
        if (self.sliderCfg.orientation == "width") {
          ...
          xypos =
            e.pageX - slider.offset().left - handle.get(0).clientWidth / 2;
        } else {
          sliderSize = slider.get(0).clientHeight;
          xypos =
            e.pageY - slider.offset().top - handle.get(0).clientHeight / 2;
        }
        xypos = (xypos * 100.0) / sliderSize;
        handle.css(
          self.sliderCfg.anchor,
          Math.min(
            Math.max(0, xypos),
            ((self.sliderCfg.range - 1) * 100.0) / self.sliderCfg.range
          ) + "%"
        );
