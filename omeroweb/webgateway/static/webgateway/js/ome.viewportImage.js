...
jQuery.fn.viewportImage = function (options) {
  return this.each(function () {
    ...
    image.wrap(
      '<div id="' +
        insideId +
        '" style="display: inline; position: absolute;" class="draggable"></div>'
    );
    ...
    var $wb_zoomIn = $(
      '<span class="wb_zoomIn" style="top: 10px;"><img src="' +
        mediaroot +
        '3rdparty/panojs-2.0.0/images/32px_plus.png" title="Zoom in" style="width: 20px;"></span>'
    ).prependTo(wrapdiv);
    var $wb_zoom11 = $(
      '<span class="wb_zoom11" style="top: 40px;"><img src="' +
        mediaroot +
        '3rdparty/panojs-2.0.0/images/32px_11.png" title="Zoom 1:1" style="width: 20px;"></span>'
    ).prependTo(wrapdiv);
    var $wb_zoomOut = $(
      '<span class="wb_zoomOut" style="top: 70px;"><img src="' +
        mediaroot +
        '3rdparty/panojs-2.0.0/images/32px_minus.png" title="Zoom out" style="width: 20px;"></span>'
    ).prependTo(wrapdiv);
    ...
    if (panbars) {
      ...
      var side_styles =
        "background-color: #ABC; filter:alpha(opacity=50); opacity: 0.5; z-index: 5; position: absolute; display: block;";
      ...
      wrapdiv.prepend(
        '<div id="' +
          ...
          '" style="' +
          side_styles_h +
          'right: 0px; _right: 0%;"><img src="' +
          mediaroot +
          '/webgateway/img/arrow_right.gif">'
      );
      wrapdiv.prepend(
        '<div id="' +
          pantopId +
      ...
      var panside_extend = function (e, min, max, mintm, maxtm, s) {
        ...
        e.queue("fx", []);
        e.children().queue("fx", []);
        e.unbind("mouseover", e.do_extend).mouseout(e.do_collapse).animate(opt);
      ...
      var panside_collapse = function (e, min, max, mintm, maxtm, s) {
        ...
        var opt2 = {};
        opt[s] = opt2[s] = min;
        e.unbind("mouseout", e.do_collapse).mouseover(e.do_extend).animate(opt);
      ...
      var panside_prepare = function (e, x, y) {
        ...
        e.do_extend = function () {
          panside_extend(e, min, max, mintm, maxtm, side);
        };
        e.do_collapse = function () {
          panside_collapse(e, min, max, mintm, maxtm, side);
        };
        e.do_collapse();
        //panside_collapse(e, min, max, mintm + 'px', maxtm+ 'px', side);
        e.mouseout(function () {
          e.removeClass("auto-val");
        });
        e.mousedown(function () {
          e.addClass("auto-val");
          self.doMove(x, y, 1, function () {
            return e.is(".auto-val");
          });
          return false;
        });
        e.mouseup(function () {
        ...
        e.display = function (show) {
          ...
          if (!show && this.css("display") != "none") {
            this.removeClass("auto-val").unbind("mouseout").hide();
          }
        };
      };
    ...
    this.showOverlay = function (url, cb, error_cb) {
      if (url) {
        ...
        var load = function () {
          overlay.unbind("error", error);
          overlay.removeClass("loading").show();
          cb && cb();
        };
        var error = function () {
          overlay.unbind("load", load);
    ...
    this.doMove = function (deltax, deltay, smooth, auto_move_cb) {
      ...
      pos.left -=
        rel.left + parseInt($.css(wrapdiv[0], "borderLeftWidth", true), 10);
      pos.top -=
        rel.top + parseInt($.css(wrapdiv[0], "borderTopWidth", true), 10);
    ...
    this.setZoom = function (val, width, height, center) {
      ...
      if (!center) {
        center = {
          x: wrapdiv.width() / 2, // pick middle by default
          y: wrapdiv.height() / 2,
        };
      }
    ...
    this.setZoomToFit = function (only_shrink, width, height) {
      ...
      var ztf = Math.min(
        (wrapwidth * 100.0) / orig_width,
        (wrapheight * 100.0) / orig_height
      );
    ...
    this.doZoom = function (increment, justDirection, center) {
      if (justDirection) {
        var t = Math.max(
          1,
          ((imagewidth + 3) * cur_zoom) / imagewidth - cur_zoom
        );
    ...
    this.setUpTiles = function (
      ...
      InfoControl.prototype.viewerZoomed = function (e) {
        if (this.dom_info) {
          if (
            nominalMagnification &&
            typeof nominalMagnification != "undefined"
          ) {
            ...
            if (scale % 1 !== 0) scale = scale.toFixed(2);
      ...
      if (viewerBean == null) {
        ...
        $(
          '<div id="' +
            viewerBeanId +
            '" class="viewer" style="width: 100%; height: 100%;" ></div>'
        ).appendTo(wrapdiv);
