(function () {
  ...
  window.OME.Histogram = function (
    ...
    // area fill
    svg.append("path").attr("class", "area").attr("opacity", 0.5);
    ...
    var plotJson = function (data, color) {
      ...
      svg.selectAll(".line").datum(data).attr("d", line).attr("stroke", color);
    ...
    var _loadAndPlot = function (
      ...
      var url =
        webgatewayUrl + "histogram_json/" + imageId + "/channel/" + theC + "/";
  ...
  window.OME.createViewportHistogram = function (
    viewport,
    chartSelector,
    checkboxSelector,
    webgatewayUrl
    ...
    viewport.bind("channelToggle", function (event, viewport, chIdx, channel) {
      ...
      if (!channel.active) {
        var active = viewport.loadedImg.channels.reduce(function (
          prev,
          ch,
          idx
        ) {
    ...
    viewport.bind("channelSlide", function (
      event,
      viewport,
      chIdx,
      start,
