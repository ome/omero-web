...
$.fn.roi_display = function (options) {
  return this.each(function () {
    ...
    var canvas_class = options.canvas_class
      ? options.canvas_class
      : "weblitz-viewport-roi";

    if (!tiles) {
      // add our ROI canvas as a sibling to the image plane. Parent is the 'draggable' div
      var $dragdiv = $viewportimg.parent();
      var canvas_name = options.canvas_name
        ? options.canvas_name
        : viewerId + "-roi";
      var $canvas = $(
        '<div id="' + canvas_name + '" class="' + canvas_class + '">'
      ).appendTo($dragdiv);
    } else {
      var canvas_name = options.canvas_name
        ? options.canvas_name
    ...
    var getArrowPath = function getArrowPath(shape) {
      ...
      if (arrowStart) {
        ...
        arrowPath +=
          " L" + arrowPoint1x + " " + arrowPoint1y + " L" + x1 + " " + y1;
      ...
      if (arrowEnd) {
        ...
        arrowPath +=
          " L" + arrowPoint1x + " " + arrowPoint1y + " L" + x2 + " " + y2;
        arrowPath += " L" + arrowPoint2x + " " + arrowPoint2y;
    ...
    var draw_shape = function (shape) {
      ...
      if (shape["type"] == "Ellipse") {
        ...
        if (shape["textValue"]) {
          newShape = paper
            .text(shape["x"], shape["y"], shape["textValue"].escapeHTML())
          ...
          if (s.type == "text") {
            ...
            strokeWidth =
              s.attr("stroke-width") > 0
                ? Math.ceil(s.attr("stroke-width") / 2)
                : 1;
    ...
    filter_rois = function (filter) {
      ...
      if (external_rois) $.merge(global_rois, external_rois);
        ...
        for (var r = 0; r < global_rois.length; r++) {
          ...
          if (filter.hasOwnProperty(global_rois[r].id)) {
            ...
            for (s = 0; s < shapes.length; s++) {
              if (
                filter[global_rois[r].id].indexOf(shapes[s].id) != -1 &&
    ...
    this.get_full_roi_json = function () {
      if (!roi_json && !external_rois) return null;
      else if (roi_json && external_rois)
        return $.merge(roi_json, external_rois);
      else if (roi_json && !external_rois) return roi_json;
      else if (!roi_json && external_rois) return external_rois;
    };
        ...
        if (external_rois[rx]["id"] == roi_id) {
          ...
          for (var sx = 0; sx < shapes.length; sx++) {
            if (shapes[sx]["id"] == shape_id) {
              console.error(
                "Shape ID " + shape_id + " already in use for ROI " + roi_id
              );
              return false;
            }
          }
    ...
    var check_ext_shape_planes = function (roi_id, shape_z, shape_t) {
      for (var rx = 0; rx < external_rois.length; rx++) {
        if (external_rois[rx]["id"] == roi_id) {
          ...
          for (var sx = 0; sx < shapes.length; sx++) {
            if (
              shapes[sx]["theZ"] == shape_z &&
              shapes[sx]["theT"] == shape_t
            ) {
              console.error(
                "Z plane " +
                  shape_z +
    ...
    /*
         ...
         */
    this.add_external_shape = function (
      roi_id,
      shape_id,
      shape_config,
      refresh_rois,
    ...
    $viewportimg.on("add_external_shape", function (
      event,
      roi_id,
      shape_id,
      shape_config,
      ...
      var check_shape_planes = check_ext_shape_planes(
        roi_id,
        shape_config["theZ"],
        shape_config["theT"]
      );
      ...
      if (refresh) {
        ...
        var hide_oroi =
          typeof hide_ome_rois !== "undefined" ? hide_ome_rois : false;
    ...
    this.remove_external_shape = function (roi_id, shape_id, refresh) {
      ...
      for (var r = 0; r < external_rois.length; r++) {
        ...
        if (roi["id"] == resolve_id(roi_id)) {
          for (var s = 0; s < roi["shapes"].length; s++) {
            ...
            if (shape["id"] == resolve_id(shape_id)) {
              ...
              if (roi["shapes"].length == 0) {
                ...
                var refresh =
                  typeof refresh_rois !== "undefined" ? refresh_rois : false;
                if (refresh) this.refresh_active_rois();
          ...
          console.warn(
            "There is no Shape with ID " + shape_id + " for ROI " + roi_id
          );
          return;
        }
      }
    ...
    this.refresh_rois = function (theZ, theT, rois_filter) {
      ...
      for (var r = 0; r < rois.length; r++) {
        ...
        for (var s = 0; s < shapes.length; s++) {
          shape = shapes[s];
          if (
            (shape.theT == this.theT - 1 ||
              typeof shape.theT === "undefined") &&
            (shape.theZ == this.theZ - 1 || typeof shape.theZ === "undefined")
          ) {
            var newShape = draw_shape(shape);
            ...
            if (newShape != null) {
              ...
              if (shape["textValue"] != null && shape["textValue"].length > 0) {
                ...
                if (shape["type"] == "Label") {
                  ...
                  if (shape["strokeColor"])
                    txt.attr({ fill: shape["strokeColor"] }); // this is Insight's behavior
                  ...
                  var text_string = formatShapeText(
                    shape["textValue"].escapeHTML()
                  );
                  ...
                  txt = paper
                    .text(
                      textx,
                      newY,
                      formatShapeText(shape["textValue"].escapeHTML())
                    )
                ...
                var txtAttr = {};
                if (shape["fontFamily"]) {
                  // model: serif, sans-serif, cursive, fantasy, monospace. #5072
                ...
                if (shape["fontStyle"]) {
                  // model: Normal, Italic, Bold, Bolditalic
              ...
              if (shape["type"] != "Label") {
                ...
                if (shape["fillColor"] && shape["type"] != "PolyLine") {
                  ...
                  if (shape["fillAlpha"]) {
                    newShape.attr({ "fill-opacity": shape["fillAlpha"] });
                  }
                } else {
                ...
                if (shape["strokeAlpha"]) {
                  newShape.attr({ "stroke-opacity": shape["strokeAlpha"] });
                }
                if (shape["strokeColor"]) {
                  newShape.attr({ stroke: shape["strokeColor"] });
                } else {
              ...
              if (shape["strokeWidth"]) {
                newShape.attr({ "stroke-width": shape["strokeWidth"] });
              }
    ...
    this.hide_rois = function (hide_ome_rois, hide_external_rois) {
      var hide_ome_rois =
        typeof hide_ome_rois !== "undefined" ? hide_ome_rois : true;
      var hide_external_rois =
        typeof hide_external_rois !== "undefined" ? hide_external_rois : true;

      if (!hide_ome_rois && !hide_external_rois) {
        console.warn(
          "HIDE_OME_ROIS and HIDE_EXTERNAL_ROIS are False, nothing to do"
        );
        return;
      }
    ...
    get_shape = function (roi_id, shape_id, roi_list) {
      for (var rx = 0; rx < roi_list.length; rx++) {
        if (roi_list[rx].id == roi_id) {
          ...
          for (var sx = 0; sx < shapes.length; sx++) {
            if (shapes[sx].id == shape_id) return shapes[sx];
          }
          console.error(
            "ROI with ID " + roi_id + "has no shape with ID " + shape_id
          );
        }
      }
      return null;
    ...
    this.restore_shape = function (roi_id, shape_id, refresh_view) {
      var refresh_view =
        typeof refresh_view !== "undefined" ? refresh_view : true;
      var backup_key = get_backup_key(roi_id, shape_id);
      if (backup_key in original_shapes_backup) {
        var original_shape = original_shapes_backup[backup_key];
        this.update_shape_text(
          roi_id,
          shape_id,
          original_shape.textValue,
          original_shape.fontFamily,
          original_shape.fontSize,
          original_shape.fontStyle,
          false
        );
        this.update_shape_config(
          roi_id,
          shape_id,
          original_shape.strokeColor,
          original_shape.strokeAlpha,
          original_shape.strokeWidth,
          original_shape.fillColor,
          original_shape.fillAlpha,
          false
        );
    ...
    /*
         ...
         */
    this.update_shape_text = function (
      roi_id,
      shape_id,
      text_value,
      font_family,
      ...
      if (!sh) sh = get_shape(roi_id, shape_id, external_rois);

      if (sh) {
        backup_shape(roi_id, shape_id, sh);

        sh.textValue =
          typeof text_value !== "undefined" ? text_value : sh.textValue;
        sh.fontFamily =
          typeof font_family !== "undefined" ? font_family : sh.fontFamily;
        sh.fontSize =
          typeof font_size !== "undefined" ? font_size : sh.fontSize;
        sh.fontStyle =
          typeof font_style !== "undefined" ? font_style : sh.fontStyle;
        ...
        console.error(
          "Unable to find a shape for ROI ID " +
            roi_id +
            " and SHAPE ID " +
            shape_id
        );
    ...
    /*
        ...
        for all values passed as "undefined".
         */
    this.update_shape_config = function (
      roi_id,
      shape_id,
      stroke_color,
      stroke_alpha,
      ...
      if (sh) {
        ...
        sh.strokeWidth =
          typeof stroke_width !== "undefined" ? stroke_width : sh.strokeWidth;
        sh.strokeAlpha =
          typeof stroke_alpha !== "undefined" ? stroke_alpha : sh.strokeAlpha;
        sh.strokeColor =
          typeof stroke_color !== "undefined" ? stroke_color : sh.strokeColor;
        sh.fillAlpha =
          typeof fill_alpha !== "undefined" ? fill_alpha : sh.fillAlpha;
        sh.fillColor =
          typeof fill_color !== "undefined" ? fill_color : sh.fillColor;
