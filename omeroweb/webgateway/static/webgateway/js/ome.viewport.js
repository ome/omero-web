...
jQuery.WeblitzViewport = function (elm, server, options) {
  ...
  var rv =
    container.WeblitzViewport ||
    (container.WeblitzViewport = new jQuery._WeblitzViewport(
      container,
      server,
      options
    ));
  ...
  this.hasSameSettings = function (other) {
    if (this.rdefs.model === other.rdefs.model) {
      for (var i in this.channels) {
        if (
          this.channels[i].active != other.channels[i].active ||
          OME.rgbToHex(this.channels[i].color) !=
            OME.rgbToHex(other.channels[i].color) ||
...
jQuery._WeblitzViewport = function (container, server, options) {
  ...
  var tileHref;
  this.self.append(
    '<div id="' + topid + '" id="' + topid + '" class="weblitz-viewport-top">'
  );
  ...
  this.viewport.append(
    '<div id="' + viewportmsgid + '" class="weblitz-viewport-msg"></div>'
  );
  ...
  this.viewportimg.bind("zoom", function (e, z) {
    _this.loadedImg.current.zoom = z;
  });
  this.zslider.gs_slider({
    orientation: "v",
    min: 0,
    max: 0,
    tooltip_prefix: "Z=",
    repeatCallback: done_reload,
  });
  this.tslider.gs_slider({
    tooltip_prefix: "T=",
    min: 0,
    max: 0,
    repeatCallback: done_reload,
  ...
  this.setZPos = function (pos) {
    if (this.getZPos() != pos) {
      // don't reload etc if we don't have to
      if (_this.loadedImg.rdefs.invertAxis) {
  ...
  var after_img_load_cb = function (callback) {
    ...
    if (
      _this.loadedImg.rdefs.projection.toLowerCase().substring(3, 0) == "int"
    ) {
      _this.zslider.get(0).setSliderRange(1, 1, _this.getPos().z + 1, false);
    } else {
      _this.zslider
        .get(0)
        .setSliderRange(1, _this.getSizes().z, _this.getPos().z + 1, false);
    }
    _this.tslider
      .get(0)
      .setSliderRange(1, _this.getSizes().t, _this.getPos().t + 1, false);
    if (callback) {
      callback();
    }
    if (_this.hasLinePlot()) {
      _this.viewportimg.one("zoom", function () {
        _this.refreshPlot();
      });
    }
  };
    ...
    if (_this.loadedImg.tiles) {
      ...
      if (typeof init_zoom === "undefined") {
        ...
        while (init_zoom > 0) {
          ...
          if (
            scaled_w < window.innerWidth - 200 ||
            scaled_h < window.innerHeight - 50
          ) {
            break;
          }
          init_zoom--;
  ...
  this.hasLinePlot = function () {
    return (
      _this.loadedImg.rdefs.projection.toLowerCase().substring(0, 6) ==
        "normal" && _this.viewportimg.get(0).overlayVisible()
    );
  };
  ...
  var pickPosHandler = function (e) {
    if (_this.getLinePlot()) {
      ...
      if (_this.getLinePlot().isVertical()) {
        pos =
          (_this.loadedImg.size.width * (e.pageX - targetpos.left)) /
          _this.viewportimg.width();
      } else {
        pos =
          (_this.loadedImg.size.height * (e.pageY - targetpos.top)) /
          _this.viewportimg.height();
      }
  ...
  this.refreshPlot = function (cb) {
    if (
      linePlot &&
      linePlot.position <=
        (linePlot.isHorizontal()
          ? this.loadedImg.size.height
          : this.loadedImg.size.width)
    ) {
      var _cb = function () {
        if (cb) {
          cb();
      ...
      var _error_cb = function () {
        hideLoading();
        showLoading("Error loading line plot!", 5);
      };
  ...
  this.refresh = function (allow_resize) {
    ...
    if (!(allow_resize || remember_allow_resize)) {
      ...
      if (a2 <= b2) {
        _this.viewport.css(
          "width",
          _this.viewport.width() - _this.viewport.position().left - 3
        );
      }
    } else {
    ...
    sli.css(
      "width",
      _this.tslider.width() - parseInt(sli.css("left"), 10) * 2 - 2
    );
    sli = jQuery(".slider-line", _this.zslider);
    btn = jQuery(".slider-btn-up", _this.zslider);
    sli.css(
      "height",
      _this.top.height() - parseInt(sli.css("top"), 10) * 2 - 2
    );
  ...
  this.setChannelReverseIntensity = function (idx, reverse, noreload) {
    ...
    console.log(
      "setChannelReverseIntensity() Deprecated in OMERO 5.4.0. Use setChannelInverted()"
    );
  ...
  this.setChannelInverted = function (idx, inverted, noreload) {
    if (_this.loadedImg.channels[idx].inverted !== inverted) {
      ...
      _this.self.trigger("channelChange", [
        _this,
        idx,
        _this.loadedImg.channels[idx],
      ]);
      if (!noreload) {
        _load();
      }
  ...
  this.setChannelActive = function (idx, act, noreload) {
    ...
    if (this.isGreyModel() && this.getProjection() != "split") {
      ...
      if (act) {
        ...
        _this.self.trigger("channelToggle", [
          _this,
          idx,
          _this.loadedImg.channels[idx],
        ]);
  ...
  this.setChannelColor = function (idx, color, noreload) {
    ...
    if (color !== _this.loadedImg.channels[idx].color) {
      ...
      if (!noreload) {
        ...
        _this.self.trigger("channelColorChange", [
          _this,
          idx,
          _this.loadedImg.channels[idx],
        ]);
  ...
  this.setChannelMinMax = function () {
    ...
    for (var i = 0; i < channels.length; i++) {
      this.setChannelWindow(
        i,
        channels[i].window.min,
        channels[i].window.max,
        true
      );
    }
    _load();
      ...
      if (!noreload) {
        _load(function () {
          if (doReset) {
            ...
            _this.viewportimg
              .get(0)
              .setZoomToFit(true, size.width, size.height);
          }
        });
      }
    }
  ...
  this.setModel = function (m, noreload) {
    ...
    if (_this.loadedImg.rdefs.model.toLowerCase().substring(0, 1) != m) {
      ...
      for (var i in _this.loadedImg.channels) {
        _this.loadedImg.current.lastModelChannels.push(
          _this.loadedImg.channels[i].active
        );
      }
  ...
  this.getModel = function () {
    return _this.loadedImg.rdefs.model
      ? _this.loadedImg.rdefs.model.toLowerCase().substring(0, 1)
      : null;
  };

  this.isGreyModel = function () {
    return _this.loadedImg.rdefs.model
      ? _this.loadedImg.rdefs.model.toLowerCase().substring(0, 1) == "g"
      : null;
  };
  ...
  this.getSizes = function () {
    ...
    var rv = { width: s.width, height: s.height, c: s.c };
    if (_this.loadedImg.rdefs.invertAxis) {
      rv.z = s.t;
      rv.t = s.z;
    } else {
  ...
  var compare_stack_entries = function (e1, e2) {
    ...
    for (var i = 0; i < e1.channels.length; i++) {
      if (
        !(
          e1.channels[i].active == e2.channels[i].active &&
  ...
  this.save_channels = function () {
    ...
    if (
      channels_undo_stack_ptr == -1 ||
      !compare_stack_entries(
        entry,
        channels_undo_stack[channels_undo_stack_ptr]
      )
    ) {
  ...
  this.undo_channels = function (redo) {
    if (channels_undo_stack_ptr >= 0) {
      ...
      for (var i = 0; i < entry.channels.length; i++) {
        this.setChannelWindow(
          i,
          entry.channels[i].windowStart,
          entry.channels[i].windowEnd,
          true
        );
  ...
  this.redo_channels = function () {
    if (
      channels_undo_stack_ptr > -1 &&
      channels_undo_stack.length - 1 > channels_undo_stack_ptr
    ) {
  ...
  this.getQuery = function (include_slider_pos, include_xy_pos, include_zoom) {
    ...
    if (include_xy_pos) {
      if (
        _this.loadedImg.tiles &&
        _this.viewportimg.get(0).getBigImageContainer()
      ) {
    ...
    if (this.hasLinePlot()) {
      query.push(
        "lp=" + (linePlot.isHorizontal() ? "h" : "v") + linePlot.position
      );
    }
      ...
      for (var j = 0; j < chs.length; j++) {
        ...
        if (t.length > 1) {
          ...
          if (range.length == 2) {
            this.setChannelWindow(
              idx,
              parseFloat(range[0], 10),
              parseFloat(range[1], 10),
              true
            );
          }
        }
  ...
  this.getRelUrl = function (append) {
    ...
    return (
      this.loadedImg.id +
      "/" +
      this.loadedImg.current.z +
      "/" +
      this.loadedImg.current.t +
      append +
      "/?" +
      this.getQuery()
    );
  ...
  this.bind = function (event, callback) {
    if (
      ...
      event == "imageLoad" ||
      event == "linePlotPos" ||
      event == "linePlotChange" ||
      event == "channelToggle" ||
      event == "channelFocus" ||
      event == "channelColorChange"
    ) {
