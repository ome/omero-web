$(function () {
  ...
  OME.emptyWellBirdsEye = function () {
    $("#well_birds_eye").empty().off("click");
  };
  ...
  OME.WellBirdsEye = function (opts) {
    ...
    function showPanel() {
      ...
      $well_details.css("height", "30%").css("display", "block");
    }
    ...
    return {
      ...
      setSelected: function (imageIds) {
        ...
        imageIds.forEach(function (iid) {
          $("img[data-imageId=" + iid + "]", $well_birds_eye).addClass(
            "ui-selected"
          );
        });
      },
      addWell: function (data) {
        ...
        data = data.filter(function (ws) {
          return ws.position !== undefined;
        });
        ...
        var html = data.map(function (ws) {
          ...
          var x =
            maxX === minX ? 0.5 : (ws.position.x.value - minX) / (maxX - minX);
          var y =
            maxY === minY ? 0.5 : (ws.position.y.value - minY) / (maxY - minY);
          return (
            '<img style="left: ' +
            x * 100 +
            "%; top: " +
